...
QR을 스캔한 사람이 **초대하고 싶은 사람의 이름과 관계**(기도제목은 선택)를 남기면 길 위의 코인이
하나씩 켜지고, 아들이 그만큼 집 쪽으로 걸어갑니다. 한 번에 여러 명을 적을 수 있고,
두 명이면 두 줄, 세 명이면 세 줄이 스프레드시트에 쌓입니다. **30개**가 다 켜지면 아들이
...
접수 내용이 스프레드시트에 그대로 쌓여서, 집회 후 명단을 정리하거나 인쇄하기 좋습니다.
...
명단을 정리하기도 쉽습니다. 취소된 초대는 지워지지 않고
`deleted` 열에 표시만 됩니다.

### 목표 개수 바꾸기

`index.html` 의 `DEFAULT_GOAL` 한 줄이 유일한 기준입니다. (기본 30)
서버 설정이 이 값을 덮어쓰지 않으니, 값이 두 군데서 어긋날 일이 없습니다.
...
- **전광판(빔 프로젝터)**: `https://.../` 를 그냥 엽니다. 4초마다 자동 갱신됩니다.
  오른쪽 아래 **[ ] 전체화면** 버튼(또는 `F` 키)을 누르면 브라우저 주소창과 탭이 사라집니다.
  버튼은 2.6초 동안 마우스를 안 움직이면 알아서 숨습니다.
- **참여자**: 우상단 QR을 스캔 → 초대장 화면(`#invite`)으로 바로 들어갑니다.
- 잘못 보냈을 때는 제출 직후 화면의 **[ 방금 보낸 것 취소 ]** 로 되돌릴 수 있습니다.

> 제출된 이름·관계·기도제목은 현황판에 그대로 공개됩니다. 안내 멘트에 이 점을 꼭 넣어주세요.
